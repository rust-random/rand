...
/// A PCG random number generator (DXSM 64/32 (LCG) variant).
///
/// Permuted Congruential Generator with 64-bit state, internal Linear
/// Congruential Generator, and 32-bit output via "double xorshift multiply"
/// output function.
///
/// This is a 64-bit LCG with explicitly chosen stream with the PCG-DXSM
/// output function. This corresponds to `pcg_engines::setseq_dxsm_64_32`
/// from pcg_cpp.
///
/// Despite the name, this implementation uses 16 bytes (128 bit) space
/// comprising 64 bits of state and 64 bits stream selector. These are both set
...
#[derive(Clone, PartialEq, Eq)]
#[cfg_attr(feature = "serde1", derive(Serialize, Deserialize))]
pub struct Lcg64Dxsm32 {
    state: u64,
    increment: u64,
}

macro_rules! impl_lcg64 {
    ($ty:ty, $out:ident, $corr:literal) => {
        impl $ty {
            ...
            #[doc = $corr]
            #[doc = ""]
            #[doc = "PCG specifies the following default values for both parameters:"]
            #[doc = ""]
            #[doc = "`state = 0xcafef00dd15ea5e5`"]
            #[doc = "`stream = 0xa02bdbf7bb3c0a7`"]
            #[doc = "Note: stream is 1442695040888963407u64 >> 1"]
...
impl_lcg64!(
    Lcg64Dxsm32,
    output_dxsm,
    "Note that while two generators with different stream parameter may be closely correlated, \
    this is [mitigated](https://numpy.org/doc/stable/reference/random/upgrading-pcg64.html) by the DXSM output function."
);

#[inline(always)]
fn output_xsl_rr(state: u64) -> u32 {
...
fn output_dxsm(state: u64) -> u32 {
    ...
    const MULTIPLY: u32 = 1284865837;

    let mut hi = (state >> 32) as u32;
    let mut lo = state as u32;

    lo |= 1;
    hi ^= hi >> 16;
    hi = hi.wrapping_mul(MULTIPLY);
    hi ^= hi >> 24;
