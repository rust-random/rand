...
mod test {
    ...
    #[test]
    // `test_monte_carlo_pi_f32` and `test_monte_carlo_pi_f64` are
    // smoke tests to ensure that next_f32 and next_f64 don't get
    // broken in silly ways.
    fn test_monte_carlo_pi_f32() {
        let mut r = thread_rng();
        ...
        let est_pi = 4. * (in_circle as f32) / (TOTAL as f32);
        let delta = (PI - est_pi).abs();
        if (PI - est_pi).abs() > EPSILON {
            println!("Expected {}, got {}", EPSILON, delta);
            panic!("Failed to converge to {} after {} trials.",
                   EPSILON, TOTAL);
        }
    }

    #[test]
    fn test_monte_carlo_pi_f64() {
        let mut r = thread_rng();
        const TOTAL: u32 = 100_000;
        ...
        let est_pi = 4. * (in_circle as f64) / (TOTAL as f64);
        let delta = (PI - est_pi).abs();
        if (PI - est_pi).abs() > EPSILON {
            println!("Expected {}, got {}", EPSILON, delta);
            panic!("Failed to converge to {} after {} trials.",
